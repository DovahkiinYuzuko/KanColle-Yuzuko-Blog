...
export function calculateShipFitBonus(shipObj, masterData) {
    ...
    if (!shipObj || !shipObj.id || !masterData || !masterData.ships || !masterData.items)
        return bonus;
    const masterShip = masterData.ships[shipObj.id] || masterData.ships[String(shipObj.id)];
    const shipId = Number(shipObj.id) || 0;
    const shipClass = masterShip?.shipClass ?? 0;
    const shipType = masterShip?.stype ?? 0;
    const originalId = masterShip?.originalId ?? masterShip?.orig ?? shipId;
    const equipList = [];
    if (shipObj.items && typeof shipObj.items === 'object') {
        for (const key of Object.keys(shipObj.items)) {
    ...
    for (const eq of equipList) {
        ...
        const antiAir = it.taiku ?? it.antiAir ?? 0;
        const scout = it.saku ?? it.scout ?? 0;
        const accuracy = it.accuracy ?? 0;
        if (itype === 11 && antiAir > 1)
            antiAirRadarCount++;
        if (itype === 11 && scout > 4)
            surfaceRadarCount++;
        if (itype === 11 && accuracy >= 8)
            accuracyRadarCount++;
    }
        ...
        for (let i = 0; i < bonusData.length; i++) {
            ...
            if ((types || ids) && bonuses) {
                ...
                for (let j = 0; j < bonuses.length; j++) {
                    const b = bonuses[j];
                    if (b.shipBase && !b.shipBase.includes(originalId))
                        continue;
                    if (b.shipClass && !b.shipClass.includes(shipClass))
                        continue;
                    if (b.shipCountry && !b.shipCountry.includes(shipClass))
                        continue;
                    if (b.shipType && !b.shipType.includes(shipType))
                        continue;
                    if (b.shipId && !b.shipId.includes(shipId))
                        continue;
                    if (b.requiresAR && !antiAirRadarCount)
                        continue;
                    if (b.requiresSR && !surfaceRadarCount)
                        continue;
                    if (b.requiresAccR && !accuracyRadarCount)
                        continue;
                    if (b.requiresId) {
                        ...
                        if (b.requiresIdNum && targetItems.length < b.requiresIdNum)
                            continue;
                        if (requireRemodel && !targetItems.some(e => e.rf >= requireRemodel))
                            continue;
                        ...
                        if (b.requiresId2) {
                            ...
                            if (!targetItems2.length)
                                continue;
                            if (requireRemodel2 && !targetItems2.some(e => e.rf >= requireRemodel2))
                                continue;
                        }
                    }
                    if (b.requiresType) {
                    ...
                    const addStat = (st) => {
                        if (!st)
                            return;
                        if (st.firePower)
                            bonus.firepower = (bonus.firepower || 0) + st.firePower;
                        if (st.torpedo)
                            bonus.torpedo = (bonus.torpedo || 0) + st.torpedo;
                        if (st.antiAir)
                            bonus.antiAir = (bonus.antiAir || 0) + st.antiAir;
                        if (st.armor)
                            bonus.armor = (bonus.armor || 0) + st.armor;
                        if (st.avoid)
                            bonus.evasion = (bonus.evasion || 0) + st.avoid;
                        if (st.scout)
                            bonus.saku = (bonus.saku || 0) + st.scout;
                        if (st.asw)
                            bonus.asw = (bonus.asw || 0) + st.asw;
                    };
                    ...
                    if (minRemodel) {
                        ...
                        if (!remodelFits.length)
                            continue;
                        if (b.num && remodelFits.length < b.num)
                            continue;
                        else if (!b.num) {
                            for (let k = 0; k < remodelFits.length; k++)
                                addStat(bStat);
                        }
                        else {
                            addStat(bStat);
                        ...
                        for (let k = 0; k < fitItems.length; k++)
                            addStat(bStat);
                    }
                    else {
                        addStat(bStat);
