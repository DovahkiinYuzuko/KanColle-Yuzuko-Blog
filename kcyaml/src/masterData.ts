 ...
 */
export async function loadFitBonusData(forceRefresh = false): Promise<any> {
  if (!forceRefresh) {
    const cached = readCache<any>(FIT_BONUS_CACHE);
...
export async function loadMasterData(forceRefresh = false, customUrl?: string): Promise<MasterData> {
  const config = loadAppConfig();
  const targetUrl = customUrl || config.urls?.masterJsonUrl || DEFAULT_MASTER_JSON_URL;

  // 1. キャッシュから読み込み (forceRefresh でない場合)
  if (!forceRefresh) {
    const cached = readCache<MasterData>(MASTER_CACHE);
    if (cached) {
  ...
  const fallbackCached = readCache<MasterData>(MASTER_CACHE);
  if (fallbackCached) {
    const fitBonus = await loadFitBonusData(false);
